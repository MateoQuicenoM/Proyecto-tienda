...
router.post('/', (req, res) => {
    ...
    // CONSULTA ROBUSTA: Una sola línea
    const sqlSubtotal = "SELECT SUM(pr.precio * pp.cantidad) AS subtotal FROM pedidos_productos pp JOIN productos pr ON pp.producto_id = pr.id WHERE pp.pedido_id = ?";
    ...
    // CONSULTA ROBUSTA: Una sola línea e incluye TODAS las columnas
      const sqlFactura = "INSERT INTO facturas (cliente_id, pedido_id, metodo_pago, subtotal, impuesto, envio, total, fecha) VALUES (?, ?, ?, ?, ?, ?, ?, NOW())";
      
      db.query(sqlFactura, [cliente_id, pedido_id, metodo_pago, subtotal, impuesto, envio, total], (err2, result) => {
        if (err2) {
...
JOIN productos pr ON pp.producto_id = pr.id
WHERE f.pedido_id = ?
    `.trim(); // ⬅️ La consulta ahora está alineada a la izquierda
